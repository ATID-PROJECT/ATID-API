...
url_moodle = (
    "webservice/rest/server.php?wstoken={0}&wsfunction={1}&moodlewsrestformat=json"
)
...
def createGroup(url_base, token, course_id, name, description):
    ...
    final_url = str(
        url_base + "/" + (settings.URL_MOODLE.format(token, function + params))
    )
    r = requests.post(final_url, data={})
    result = r.json()
...
def hasSuccessConnected(db, url_base, network_id, token, current_user):
    function = "core_webservice_get_site_info"

    with urllib.request.urlopen(
        str(url_base + "/" + (url_moodle.format(token, function)))
    ) as url:
...
def makeCourse(db: Graph):
    ...
    if has_connected:
        ...
        return (
            jsonify({"message": "`url` e `token` são obrigatórios.", "status": 400}),
            400,
        )
...
def create_course(db, current_user, network_id, fullname, shortname):
    try:
        ...
        all_questions = db.run(
            "MATCH (p:User{email:'%s'})-[r1]-(net:Network{id:'%s'})-[r2:HAS_QUESTIONS]-(q) return q"
            % (current_user, network_id)
        ).data()
...
def get_user(url, token, id):
    ...
    print(
        f"{url}/{url_moodle.format( token, function)}&field=id&values[0]={id}",
        file=sys.stderr,
    )
    with urllib.request.urlopen(
        f"{url}/{url_moodle.format( token, function)}&field=id&values[0]={id}"
...
def quiz_dates():
    try:
        ...
        for nt in not_trigged:
            ...
            if (
                "open_date" in trigger
                and datetime.datetime.strptime(
                    trigger["open_date"], "%Y-%m-%d %H:%M:%S"
                )
                <= datetime.datetime.now()
                and "allow_open_date" in trigger
                and trigger["allow_open_date"] == "True"
            ):
                network = db.run(
                    "MATCH (n:Network)-[]-(:Quiz{{uuid: '{0}' }}) RETURN n".format(
                        trigger["uuid"]
                    )
                ).data()[0]["n"]
                quiz_instance = db.run(
                    "MATCH (qi:QuizInstance) WHERE qi.id_quiz = '{0}' RETURN qi".format(
                        trigger["uuid"]
                    )
                ).data()[0]["qi"]
                courses = db.run(
                    "MATCH (n:Network)-[]-(c:Course) WHERE n.id = '{0}' RETURN c".format(
                        network["id"]
                    )
                ).data()
                ...
                for c in courses:
                    ...
                    for suggestion in old_nodes:
                        ...
                        node2 = db.run(
                            "MATCH (qi:ChatInstance) WHERE qi.id_chat = '{0}' RETURN qi".format(
                                suggestion
                            )
                        ).data()
                        ...
                        if node1 != None and len(node1) > 0:
                            ...
                            node = node2[0]["qi"]
                        users = getGroupUsers(
                            network["url"], network["token"], int(node["id_group"])
                        )
...
def getStudents(db: Graph):
    ...
    result = db.run(
        f"MATCH (p:User{{email:'{current_user}'}})-[r1]-(net:Network{{id:'{network_id}'}}) return net"
    ).data()
    activity = db.run(
        f"MATCH (course:Course{{id:{course_id}}})-[r1]-(activity:{type_capitalize}Instance{{id_{type_lower}:'{activity_id}'}}) return activity"
    ).data()
    ...
    with urllib.request.urlopen(
        f"{result['url']}/{url_moodle.format( result['token'] , function)}&groupids[0]={activity['id_group']}"
    ) as url:
        data = json.loads(url.read().decode())

        if "exception" in data:
            return (
                jsonify({"message": "`url` e `token` inválidos.", "status": 400}),
                400,
            )
...
def getFunctionMoodle():
    ...
    with urllib.request.urlopen(
        str(source_moodle + (url_moodle.format(token, function)))
    ) as url:
        data = json.loads(url.read().decode())
...
def save_SubNetwork(db: Graph):
    dataDict = json.loads(request.data)
    current_user = get_jwt_identity()
    SubNetwork.update_by_user(
        db,
        current_user,
        dataDict["id_activity"],
        dataDict["id_subnetwork"],
        dataDict["data"],
    )
...
def log_network(db: Graph):
    ...
    result = NetworkUserLog.query.filter_by(network_id=id_network).order_by(
        NetworkUserLog.created_on.desc()
    )
...
def get_by_id_SubNetwork(db: Graph):
    current_user = get_jwt_identity()
    result = SubNetwork.fetch_by_id(
        db,
        current_user,
        request.args.get("id_activity"),
        request.args.get("id_subnetwork"),
    )
    if result:
        result = SubNetwork.fetch_by_id(
            db,
        ...
        sub.all_data = "[]"
        db.push(sub)
        SubNetwork.create_relationship(
            db,
            current_user,
            request.args.get("id_activity"),
            request.args.get("id_subnetwork"),
...
def save_Network(db: Graph):
    ...
    current_user = get_jwt_identity()
    if "data" in dataDict:
        result = Network.update_by_user(
            db, current_user, dataDict["id"], dataDict["data"], getCurrentDate()
        )
    if "name" in dataDict:
        result = Network.update_name_by_user(
            db, current_user, dataDict["id"], dataDict["name"], getCurrentDate()
        )
...
def get_by_id_course(db: Graph):
    ...
    result = db.run(
        "MATCH (p:User{email:'%s'})-[r1]-(net:Network)-[r2]-(course:Course{id:%s}) return net"
...
@account_controller.route(
    "/users/activity/<string:network_id>/duplicate", methods=["POST"]
)
@jwt_required
def duplicate_Network(db: Graph, network_id):
    ...
    atividade = make_network(
        db,
        current_user,
        course_name=f"{result['name']}_cópia",
...
def register(db: Graph):
    ...
    if (
        not dataDict["email"]
        or not dataDict["first_name"]
        or not dataDict["last_name"]
        or not dataDict["password"]
    ):
        return (
            jsonify(
                {
                    "error": "`email`, `first_name`, `last_name` e `password` são obrigatórios."
                }
            ),
            400,
        )

    usuario = User.fetch_by_email(
        graph=db, email=dataDict["email"]
    )  # Usuario.query.filter_by(email=username,senha=getHash512(password)).first()
    if usuario:
...
def login(db: Graph):
    ...
    try:
        dataDict = json.loads(request.data)
        usuario = User.fetch_by_email_and_password(
            db, email=dataDict["email"], password=getHash512(dataDict["password"])
        )
        if not usuario:
            return jsonify({"error": "Email ou senha inválido."}), 400

        user = UserObject(
            username=usuario.email, role="Admin", permissions=["foo", "bar"]
        )
